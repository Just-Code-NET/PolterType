...
///
/// Not enumerated at runtime: the bundled set is curated while
/// user-side TOMLs are best-effort, and directory enumeration is one
/// more failure mode (permissions, non-UTF-8 names) on the startup
/// path.
///
...
/// it is a floor under a query that went wrong. A bundled mapping
/// that is right for the wrong variant still beats one missing half its
/// alphabet.
