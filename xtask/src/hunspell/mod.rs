...
//! 2026-05-07).
//!
//! Covers `SFX`/`PFX` rules of `<strip> <add> <condition>` shape;
...
//! PFX × SFX (uk_UA has no PFX rules and the others a handful), and the
//! spell-checker concerns — `ICONV`/`OCONV`, `MAP`, `REP`, `BREAK`,
//! `TRY` — which are ignored while parsing.
//!
//! So it is a **lossy** port: most prose vocabulary, not the corner
//! cases. `data/wordlists/<lang>-extras.txt` is the escape hatch.
