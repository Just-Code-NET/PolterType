...
> **Status:** v0.18.1 — out of beta since v0.1.0. Works end-to-end on
...
| Linux (X11)     | working, and needs **no setup script at all** — XInput2 listener + XTest emitter need no `input`-group membership. Measured on Xfce, LXQt, Cinnamon, icewm, openbox and fluxbox. **MATE is the exception**: its settings daemon owns the keyboard layout and reports it inconsistently, so PolterType stands down there. |
